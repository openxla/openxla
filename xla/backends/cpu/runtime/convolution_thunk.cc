...
absl::StatusOr<std::unique_ptr<ConvolutionThunk>> ConvolutionThunk::FromProto(
  ...
  return Create(std::move(info), std::move(options), std::move(input_buffer),
                input_shape, std::move(kernel_buffer), kernel_shape,
                std::move(output_buffer), output_shape, dnums, window,
                feature_group_count);
}
