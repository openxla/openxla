...
class DynamicParameterBinding {
 public:
  enum class Target { kParam = 1, kOutput = 2 };
  ...
  struct DynamicParameter {
    ...
    ShapeIndex parameter_index;  // the index of the parameter
  };
  ...
  struct DynamicDimension {
    ...
    ShapeIndex target_index;  // the subshape index of the target
    int64_t dimension;        // the dimension number in the subshape

    // Returns dynamic dimensions descriptor for the parameter.
    static DynamicDimension Param(int64_t param_num, ShapeIndex param_index,
                                  int64_t param_dimension) {
      return {Target::kParam, param_num, param_index, param_dimension};
    }

    // Returns dynamic dimensions descriptor for the output.
    static DynamicDimension Output(int64_t output_num, ShapeIndex output_index,
                                   int64_t output_dimension) {
      return {Target::kOutput, output_num, output_index, output_dimension};
    }

    // "friend" keyword are added so these functions can be found by ADL.
    template <typename H>
    friend H AbslHashValue(H h, const DynamicDimension& m) {
      return H::combine(std::move(h), m.target, m.target_num, m.target_index,
                        m.dimension);
    }
