 ...
 public:
  ...
  CopyRemover(const HloModule& module, const HloAliasAnalysis& alias_analysis,
    ...
    for (const HloBuffer& buffer : alias_analysis.buffers()) {
      ...
      // strict weak ordering. We cannot invoke absl::c_sort() which requires
      // strict weak ordering. IsDefinedBefore() is strict partial ordering
      // which can be used for doing a topological sort.
      auto topological_sort = [this](std::vector<const HloValue*>& values) {
        auto begin = values.begin();
        auto const end = values.end();
        while (begin != end) {
          // If none of the remaining items are defined before `a`, `a` is
          // a candidate for being scheduled. We partition the
          // remaining items into candidates and non-candidates.
          auto const next_begin =
              std::stable_partition(begin, end, [&](const HloValue* a) {
                return std::none_of(begin, end, [&](const HloValue* b) {
                  return ordering_->IsDefinedBefore(*b, *a);
                });
              });
          begin = next_begin;
        }
